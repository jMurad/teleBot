...
func (dbpg *DatabasePG) DBinit() {
	var err error

	dbpg.PoolConn, err = pgxpool.Connect(context.Background(), os.Getenv("PG_URL"))
	if err != nil {
...
func (dbpg *DatabasePG) AddToLog(userid, firstname, msg string) {
	now := time.Now().Local()
	conn, err := dbpg.PoolConn.Acquire(context.Background())
	if err != nil {
...
func (dbpg *DatabasePG) UsersOfPeriod(beginTime, endTime time.Time) []logClient {
	conn, err := dbpg.PoolConn.Acquire(context.Background())
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Error acquiring connection: %v\n", err)
		os.Exit(1)
...
func (dbpg *DatabasePG) MsgUserOfPeriod(user string, beginTime, endTime time.Time) []logClient{
	conn, err := dbpg.PoolConn.Acquire(context.Background())
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Error acquiring connection: %v\n", err)
		os.Exit(1)
...
func (dbpg *DatabasePG) ParseAdminMsg(msg string) bool {
	...
	if uop.MatchString(msg) {
		...
		dbpg.lCl = dbpg.UsersOfPeriod(beginPeriod, endPeriod)
		...
		dbpg.lCl = dbpg.MsgUserOfPeriod(user, beginPeriod, endPeriod)
		return true
	}
	return false
}

func (dbpg *DatabasePG) PrettyLog() string {
	var out string
	for _, l := range dbpg.lCl {
